plot.ordinDNA <- function(x, textcex = 0.7, plotCol = "default", majorAxes = c(1,2), ...){
	#Colours
	...
	points(centroids[,2], centroids[,3], pch = 21, bg = plotCol[as.numeric(sort(unique(sppVecFac)))])
	points(mat[,1], mat[,2], pch=22, bg = plotCol[as.numeric(sppVecFac)], cex = 0.5)
	text(centroids[,2], centroids[,3], labels = sort(unique(sppVector)), cex = textcex)
