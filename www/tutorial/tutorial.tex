...
%%%%%%%%%%%%% NEED TO COMPLETELY REDO THIS BIT
...
\begin{console}
table(monoBoot$results[match(anoSpp, unique(anoSpp))], useNA = "ifany")
\end{console}
...
The principle of sliding window analysis is fairly simple---a DNA alignment is divided up into multiple smaller fragments throughout the length of the region in question. We can then assess different types of information content though the alignment, measure nucleotide diversity \citep{Roe.Sper.2007}, or investigate phylogenetic character conflict \citep{Crui.2011}.
...
List of 13
 ...
 $ comp_depth_out: num [1:120] 0.588 0.471 0.471 0.471 0.353 ...
 $ comp_out      : num [1:120] 0.355 0.29 0.29 0.29 0.226 ...
 $ pos_tr_out    : num [1:120] 1 4 7 10 13 16 19 22 25 28 ...
 $ noncon_out    : num [1:120] 0.333 0.394 0.212 0.212 0.303 ...
 $ nd_out        : num [1:120] 8 9 7 6 5 6 6 8 9 9 ...
 $ zero_out      : num [1:120] 0.0303 0.0398 0.0265 0.0246 0.036 ...
 $ dist_mean_out : num [1:120] 0.0742 0.0741 0.0804 0.083 0.0796 ...
...
\verb|comp_out| and \verb|comp_depth_out| compare the clades of the trees in the window to the trees made from the full dataset. The difference is that \verb|comp_depth_out| considers only the clades that are closer to the tips of the tree than the median node depth, while \verb|comp_out| looks at all of them.
...
\verb|nd_out| summarises nucleotide diagnostic sites (i.e., pure, simple, characteristic attributes) for each and all species; handy for primer design.
...
Finally, the last three objects in the list are used by \fun{plot()} in creating the plots  described below.
...
\begin{Routput}
   position mean_distance zero_noncon zero_distances diag_nuc monophyly
67      199    0.10636352   0.1212121     0.01136364        7 0.8636364
66      196    0.10495195   0.1212121     0.01136364        6 0.8636364
63      187    0.10448748   0.1212121     0.01136364        7 0.8636364
68      202    0.10347462   0.1212121     0.01136364        7 0.8636364
65      193    0.10222185   0.1212121     0.01136364        7 0.8636364
64      190    0.10215716   0.1212121     0.01136364        7 0.8636364
69      205    0.10059984   0.1212121     0.01136364        7 0.8636364
60      178    0.09360184   0.2727273     0.01704545        7 0.8181818
62      184    0.09350650   0.2727273     0.01704545        7 0.8181818
61      181    0.09210280   0.2727273     0.01704545        6 0.8181818
   clade_comparison clade_comp_shallow
67        0.3870968          0.5882353
66        0.3870968          0.5882353
63        0.3548387          0.5294118
68        0.3870968          0.5882353
65        0.3225806          0.5294118
64        0.3225806          0.5294118
69        0.3870968          0.5882353
60        0.3225806          0.4705882
62        0.2903226          0.4117647
61        0.2903226          0.4117647
\end{Routput}

This shows the same conclusion that we drew from the graphical representation of the data, namely that the windows around the 200 bp mark are particularly good by most of the criteria. The default behaviour of \fun{rankSlidWin()} is that it returns the best window sorted on highest mean distance, but the other criteria can be used to rank the windows. See the help page of \fun{rankSlidWin()} for more details.
...
\begin{console}
anoBox <- slideBoxplots(anoteropsis, anoSpp, 50, interval="codons", 
method="nonCon")
plot(anoBox)
\end{console}
...
When plotted, this function produces the graphic shown in Figure \ref{slideBoxplots.fig}. Unfortunately, there is very little difference between the intra- and interspecific distances at the 50 bp window size. Even the windows around the 200 bp position display significant overlap between the two measures, showing that there is no ``barcoding gap'' with this size of DNA fragment.
...
\begin{console}
weevilsBox <- slideBoxplots(weevils, weevilSpp1, 50, interval="codons", 
method="nonCon")
plot(weevilsBox)
\end{console}
